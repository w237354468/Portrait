...
public class HbaseUtil {
  ...
  public static void putMoreData(String tableName, List<Put> puts) throws IOException {
    Table table = conn.getTable(TableName.valueOf(tableName));

    table.put(puts);
    table.close();
    System.out.println("puts ok");
  }

  public static Put getPut(String rowKey, String familyName, Map<String, String> data)
      throws IOException {
    byte[] rowkeyByte = Bytes.toBytes(rowKey);
    Put newPut = new Put(rowkeyByte);
    if (data != null) {
  ...
  public static void putData(
      String tableName, String rowKey, String familyName, String column, String data)
      throws IOException {
    Table table = conn.getTable(TableName.valueOf(tableName));
